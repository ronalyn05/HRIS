...
function Dashboard() {
  ...
  useEffect(() => {
    // Function to fetch number of employees
    const fetchNumberOfEmployees = async () => {
      try {
        const response = await axios.get('/newHireEmp'); // Make HTTP GET request to fetch employees
        const employees = response.data; // Get employees from response data
        setNumberOfEmployees(employees.length); // Set number of employees in state
      } catch (error) {
        console.error('Error fetching number of employees:', error);
      }
    };

    // Call the function to fetch number of employees
